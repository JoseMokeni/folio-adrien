<x-layout-dashboard>
    ...
    <div class="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-2 text-secondary-600">
                        Modifiez les informations de <span class="font-semibold text-primary-600">"{{ $account->name }}"</span>
                    </p>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                >
                                    <option value="eur" {{ (old('currency', strtolower($account->currency)) === 'eur') ? 'selected' : '' }}>🇪🇺 EUR (€) - Euro</option>
                                    <option value="usd" {{ (old('currency', strtolower($account->currency)) === 'usd') ? 'selected' : '' }}>🇺🇸 USD ($) - Dollar Américain</option>
                                    <option value="tnd" {{ (old('currency', strtolower($account->currency)) === 'tnd') ? 'selected' : '' }}>🇹🇳 TND (د.ت) - Dinar Tunisien</option>
                                    <option value="gbp" {{ (old('currency', strtolower($account->currency)) === 'gbp') ? 'selected' : '' }}>🇬🇧 GBP (£) - Livre Sterling</option>
                                    <option value="cad" {{ (old('currency', strtolower($account->currency)) === 'cad') ? 'selected' : '' }}>🇨🇦 CAD (C$) - Dollar Canadien</option>
                                    <option value="mad" {{ (old('currency', strtolower($account->currency)) === 'mad') ? 'selected' : '' }}>🇲🇦 MAD (د.م) - Dirham Marocain</option>
                                </select>
        ...
        <!-- Danger Zone -->
        <div class="mt-8 glass-morphism rounded-2xl p-6 border border-danger-200/30 bg-gradient-to-br from-danger-50/30 to-danger-100/30">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-danger-700 mb-4">
                        La suppression de ce compte est irréversible. Toutes les transactions associées seront également supprimées.
                    </p>
