                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-secondary-600">
                            Modifiez les détails de <span
                                class="font-semibold text-primary-600">"{{ $movement->label }}"</span>
                        </p>
                    </div>
                ...
                <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="amount" name="amount" step="0.01" required
                                    value="{{ old('amount', $movement->amount) }}" placeholder="0.00"
                                    class="block w-full pl-10 pr-4 py-3 bg-white/50 rounded-xl border border-white/20 text-secondary-900 placeholder-secondary-500 shadow-soft backdrop-blur-sm focus:border-warning-400 focus:ring-2 focus:ring-warning-400/30 focus:bg-white/70 transition-all duration-300 sm:text-sm">
                            </div>
                            @error('amount')
                                <p
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <textarea id="description" name="description" rows="3" placeholder="Ajoutez des détails sur cette transaction..."
                                class="block w-full pl-10 pr-4 py-3 bg-white/50 rounded-xl border border-white/20 text-secondary-900 placeholder-secondary-500 shadow-soft backdrop-blur-sm focus:border-warning-400 focus:ring-2 focus:ring-warning-400/30 focus:bg-white/70 transition-all duration-300 sm:text-sm resize-none">{{ old('description', $movement->description) }}</textarea>
                        </div>
                        @error('description')
                            <p
                                class="mt-2 text-sm text-danger-600 bg-danger-50 rounded-lg px-3 py-2 border border-danger-200">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <select id="nature" name="nature" required
                                    ...
                                    <option value="income"
                                        {{ old('nature', $movement->nature) === 'income' ? 'selected' : '' }}>💰
                                        Revenu</option>
                                    <option value="expense"
                                        {{ old('nature', $movement->nature) === 'expense' ? 'selected' : '' }}>💸
                                        Dépense</option>
                                </select>
                                ...
                                <select id="account" name="account" required
                                    ...
                                    @foreach ($accounts as $account)
                                        <option value="{{ $account->id }}"
                                            {{ old('account', $movement->account_id) == $account->id ? 'selected' : '' }}>
                                ...
                                <select id="category" name="category"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('category', $movement->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
