        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 p-5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">Total Revenus</p>
                        <p class="text-2xl font-bold text-green-600">
                            +{{ number_format($movements->where('nature', 'income')->sum('amount'), 2) }} €
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-600">
                            -{{ number_format($movements->where('nature', 'expense')->sum('amount'), 2) }} €
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <a href="{{ route('movement.create') }}"
                    class="inline-flex items-center gap-2 rounded-md bg-emerald-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 transition w-full sm:w-auto justify-center">
                ...
                @forelse ($movements as $movement)
                    <a href="{{ route('movement.show', $movement->id) }}"
                       class="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 hover:bg-gray-50 transition gap-3">
                        <div class="flex items-center gap-4">
                            <div class="p-2 rounded-full {{ $movement->nature === 'income' ? 'bg-green-100' : 'bg-red-100' }}">
                                <svg class="w-6 h-6 {{ $movement->nature === 'income' ? 'text-green-600' : 'text-red-600' }}"
                        ...
                        <div class="flex items-center gap-2 sm:gap-4">
                            <span class="text-sm font-bold {{ $movement->nature === 'income' ? 'text-green-600' : 'text-red-600' }}">
